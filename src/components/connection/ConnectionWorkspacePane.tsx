...
export function ConnectionWorkspacePane({
	connectionUuid,
	isActive,
}: {
	...
	const handleCloseConnection = useCallback(async () => {
		if (!uuid) return;
		const remaining = activeIds.filter((id) => id !== uuid);

		removeActive(uuid);
		try {
			await api.pool.disconnect(uuid);
		} catch (error) {
			console.error("Failed to disconnect pool:", error);
		}

		const nextId = remaining[0] || null;
		setActive(nextId);
		if (nextId) {
			navigate(`/connections/${nextId}`);
		} else {
			navigate("/");
		}
				...
				{tab.loading ? (
					...
					<div className="h-[65vh] overflow-hidden">
						<DataTable
							...
							virtualize={tab.data.data.length > 50}
							sortable
							sort={tab.sort}
